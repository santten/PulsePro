...
# MQTT
MQTT_BROKER = "192.168.3.253"
PORT = 1883
TOPIC = "topic/test"


# Define MQTT client
client = MQTTClient("pico_client", MQTT_BROKER, PORT)

# Connect to the MQTT broker
client.connect()

def publish_MQTT(message, TOPIC="topic/test"):
    client.publish(TOPIC, message)
    print("Published to MQTT:", message)
    
publish_MQTT("I AM SO HAPPY")
...
def kubios_send(intervals):
    ...
    print(pure_interval_list)
    print(len(pure_interval_list))
        ...
        if counted_tick_count > tick_diff_min:
            ...
            if 40 < BPM < 250:
                ...
                if not_too_much_variety:
                    oled.text(f"HR: {(sum(last_three_bpms) / len(last_three_bpms)):.1f} BPM", xpos, line_height, text_color)
                else:
